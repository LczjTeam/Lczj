jQuery(function () {
        ...
        if (e !== k) {
            switch (c.removeClass("state-" + k), c.addClass("state-" + e), k = e) {
                ...
                case"confirm":
                   // uploader.reset();
                    n.refresh();
                    for(var i= 0 ; i < n.getFiles().length;i++){
                        n.removeFile(n.getFiles()[i]);
                        console.log(n.getFiles()[i].id);

                        var $li = $("#"+n.getFiles()[i].id);
                        $li.remove();
                    }
                   ...
                   // n.stats("pedding");
                    ...
                    break;
                case"finish":
                    a = n.getStats(), a.successNum ? console.log("上传成功") : (k = "done", location.reload())
            }
        }
    }
    ...
    n = WebUploader.create({
        ...
        server: "../good/upload",
        fileNumLimit: 300,
        fileSizeLimit: 102400000000000,
        fileSingleSizeLimit: 10240000000
        ...
        if(response._raw != null &&  response._raw!='undefined'){
            ph = response._raw;
        }

        console.log(ph)
        var str  = '<div class="col-sm-12" id="'+ph+'">'+
                        '<div class="col-sm-8">'+
                            '<input name="fileName" value="'+ph+'" readonly style="background-color: white;border: none;outline: none;" class="form-control" type="hidden">'+
                            '<input  value="'+ph+'.png" readonly style="background-color: white;border: none;outline: none;" class="form-control" type="text">'+
                        '</div>'+
